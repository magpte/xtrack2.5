#!/usr/bin/env python3
"""
tile_bundle_lz4hc.py

Packs map tile images into "TBND" bundle files using Adaptive 3-Tier
Micro-Chunk LZ4HC-12 compression (Scheme 1 + Scheme 3 + LZ4HC).

Features:
  - Adaptive 3-Tier Micro-Palette + G-Decorrelation.
  - LZ4HC Level 12 compression backend.
  - Multi-process parallel packaging & 512-byte sector alignment.
  - Fixed 100x100 tile block size matching MCU BUNDLE_BLOCK_SIZE.

Usage:
    python tile_bundle_lz4hc.py <input_dir> <output_dir> [--workers N] [--gcj02-to-wgs84] [--tencent]
"""
import argparse
import math
import os
...
from tile_lz4hc_adaptive_encode import encode_tile_bytes, load_source_image
...
def _encode_block(args):
    """Worker process: packs one bundle block (.tbnd) with LZ4HC-12 tiles."""
...
def main():
    ...
    print(f"Scanning {args.input_dir} ... (Encoder: LZ4HC-12 Adaptive Lossless, BlockSize: {BLOCK_SIZE}x{BLOCK_SIZE})", file=sys.stderr)
