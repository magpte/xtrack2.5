    ...
    const HUF_DTable* DTable)
{
    ...
    {   const BYTE* const istart = (const BYTE*) cSrc;
        ...
        /* On ARM Cortex-M4 (32-bit in-order MCU): decode the 4 streams sequentially (single-stream pipeline).
         * This completely eliminates 4-stream register spilling (24 variables -> 3 variables),
         * keeping all state in r4-r7 registers for 100% register residency.
         */
        CHECK_F( BIT_initDStream(&bitD1, istart1, length1) );
        HUF_decodeStreamX1(op1, &bitD1, opStart2, dt, dtLog);
        if (!BIT_endOfDStream(&bitD1)) return ERROR(corruption_detected);

        CHECK_F( BIT_initDStream(&bitD2, istart2, length2) );
        HUF_decodeStreamX1(op2, &bitD2, opStart3, dt, dtLog);
        if (!BIT_endOfDStream(&bitD2)) return ERROR(corruption_detected);

        CHECK_F( BIT_initDStream(&bitD3, istart3, length3) );
        HUF_decodeStreamX1(op3, &bitD3, opStart4, dt, dtLog);
        if (!BIT_endOfDStream(&bitD3)) return ERROR(corruption_detected);

        CHECK_F( BIT_initDStream(&bitD4, istart4, length4) );
        HUF_decodeStreamX1(op4, &bitD4, oend,     dt, dtLog);
        if (!BIT_endOfDStream(&bitD4)) return ERROR(corruption_detected);
